import { managedElections } from './admin'
import {
...
const organizations: Record<string, string> = {
  'student-council-2024': 'ABC University',
  'tech-club-president': 'Campus Tech Club',
  'cultural-fest-committee': 'Cultural Committee',
...
function defaultActivity(election: Election): ManageActivityItem[] {
  if (election.status === 'ended') {
    return [
      { id: 'created', title: 'Election created', time: '12 days ago', type: 'created' },
      { id: 'opened', title: 'Voting opened', time: '8 days ago', type: 'opened' },
      { id: 'milestone', title: '1,000 votes reached', time: '7 days ago', type: 'milestone' },
      { id: 'ended', title: 'Results finalized', time: 'Yesterday', type: 'ended' },
    ]
  }

  if (election.status === 'live') {
    return [
      { id: 'created', title: 'Election created', time: '2 days ago', type: 'created' },
      { id: 'opened', title: 'Voting opened', time: '1 day ago', type: 'opened' },
      { id: 'm500', title: '500 votes reached', time: '8 hours ago', type: 'milestone' },
      { id: 'm1000', title: '1,000 votes reached', time: '3 hours ago', type: 'milestone' },
    ]
  }

  return [
    { id: 'created', title: 'Election created', time: '2 days ago', type: 'created' },
    { id: 'scheduled', title: 'Election scheduled', time: 'Yesterday', type: 'updated' },
  ]
}
  ...
  return {
    ...
    hourlyActivity:
      election.status === 'live' || election.status === 'ended'
        ? liveHourly.map((point) => ({ ...point }))
        : emptyHourly.map((point) => ({ ...point })),
    activityLog: defaultActivity(election),
  }
}

export function hourlyForStatus(status: ElectionStatus): HourlyPoint[] {
...
export function getRegisteredVoters(election: Election) {
  ...
  const managed = managedElections.find((item) => item.electionId === election.id)
  return managed?.registeredVoters ?? 0
}
