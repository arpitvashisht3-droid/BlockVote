...
export function mapApiCandidateToCandidate(c: any): Candidate {
  return {
    id: c.id,
    name: c.name,
    department: c.department || 'General',
    about: c.about || '',
    achievements: Array.isArray(c.achievements) ? c.achievements : defaultCandidateAchievements(c.department || 'General', c.position),
    position: c.position || 'Candidate',
    removed: false,
    onchainId: c.onchainId !== undefined ? c.onchainId : null,
  }
}

export function mapApiElectionToElection(apiEl: any): Election {
  return {
    id: apiEl.id,
    title: apiEl.title,
    description: apiEl.description || '',
    detailsDescription: apiEl.detailsDescription || apiEl.description || '',
    status: apiEl.status || 'upcoming',
    electionCode: apiEl.electionCode || apiEl.slug || `BLC-${apiEl.id.slice(0, 4)}`,
    startDate: apiEl.startDate ? new Date(apiEl.startDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' }) : 'TBD',
    endDate: apiEl.endDate ? new Date(apiEl.endDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' }) : 'TBD',
    voterStatus: 'Eligible',
    timeLabel: apiEl.status === 'live' ? 'Live Now' : apiEl.status === 'upcoming' ? 'Upcoming' : 'Ended',
    organization: apiEl.organization || 'BlockVote Platform',
    electionType: apiEl.electionType || 'General Election',
    onchainId: apiEl.onchainId !== undefined ? apiEl.onchainId : null,
    published: apiEl.published !== undefined ? apiEl.published : true,
    candidates: (apiEl.candidates || []).map(mapApiCandidateToCandidate),
    thumbnail: { icon: 'landmark', tone: 'navy' },
  }
}
