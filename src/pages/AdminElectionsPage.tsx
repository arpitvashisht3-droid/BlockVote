...
import { mapApiElectionToElection, type ElectionType } from '../data/elections'
import { defaultSettings } from '../data/votingSettings'
...
export function AdminElectionsPage() {
  ...
  const [items, setItems] = useState<ElectionManagement[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [archiveTarget, setArchiveTarget] = useState<ElectionManagement | null>(null)
  ...
  useEffect(() => {
    ...
    fetchApiElections(DEFAULT_ADMIN_TOKEN)
      .then((data) => {
        if (isMounted) {
          const mapped: ElectionManagement[] = data.map((apiEl) => {
            const election = mapApiElectionToElection(apiEl)
            return {
              id: election.id,
              election,
              organization: election.organization || 'BlockVote Platform',
              paused: apiEl.paused ?? false,
              archived: false,
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-6xl py-12 text-center text-navy-muted">
        Loading elections from backend API...
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-6xl py-12 text-center text-red-600 font-semibold">
    ...
    <div className="mx-auto max-w-6xl space-y-6 overflow-x-hidden">
      ...
      {visible.length > 0 ? (
        <AdminElectionTable
          ...
          onDuplicate={() => {
            showToast('Election duplication is not available in backend API mode.')
          }}
      ...
      <ElectionControlModal
        ...
        onConfirm={() => {
          showToast('Status management (archive) API is not yet implemented.')
