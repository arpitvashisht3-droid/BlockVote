...
type FinalizeResultsModalProps = {
  open: boolean
  onClose: () => void
  onConfirm: () => void
}
...
export function FinalizeResultsModal({
  ...
  return (
    <Modal open={open} title="Finalize Results?" onClose={onClose}>
      {open ? (
        <div>
          <div className="flex items-start justify-between gap-3">
            <h2 className="text-lg font-bold text-navy">Finalize Results?</h2>
            <button
              type="button"
          ...
          <p className="mt-3 text-sm text-navy-muted">
            Once results are finalized, the election result becomes the official
            final result.
          </p>
