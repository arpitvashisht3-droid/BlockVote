import { BadgeCheck, Pause, Plus, Radio, TrendingUp } from 'lucide-react'
import type { ManageActivityItem } from '../../../data/manageElection'

const icons = {
  created: Plus,
  opened: Radio,
  milestone: TrendingUp,
  paused: Pause,
  ended: BadgeCheck,
  updated: BadgeCheck,
} as const

type ElectionActivityProps = {
  items: ManageActivityItem[]
}

export function ElectionActivity({ items }: ElectionActivityProps) {
  return (
    <section className="card p-5 sm:p-6">
      <h2 className="text-lg font-bold text-navy">Election Activity</h2>
      <ol className="mt-4 space-y-4">
        {items.map((item, index) => {
          const Icon = icons[item.type]
          return (
            <li key={item.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                <span className="flex size-9 items-center justify-center rounded-lg bg-accent-soft text-accent">
                  <Icon className="size-4" aria-hidden="true" />
                </span>
                {index < items.length - 1 ? (
                  <span className="mt-1 w-px flex-1 bg-border" aria-hidden="true" />
                ) : null}
              </div>
              <div className="pb-2">
                <p className="text-sm font-semibold text-navy">{item.title}</p>
                <p className="mt-1 text-xs text-navy-muted">{item.time}</p>
              </div>
            </li>
          )
        })}
      </ol>
    </section>
  )
}
