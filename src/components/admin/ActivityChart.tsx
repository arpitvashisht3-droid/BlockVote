import { votingActivity } from '../../data/admin'
...
export function ActivityChart() {
  ...
  const maxVotes = Math.max(...votingActivity.map((point) => point.votes))
  const plotWidth = width - pad.left - pad.right
  const plotHeight = height - pad.top - pad.bottom

  const points = votingActivity.map((point, index) => {
    const x =
      pad.left +
      (votingActivity.length === 1
        ? plotWidth / 2
        : (index / (votingActivity.length - 1)) * plotWidth)
    ...
    <section className="card p-5 sm:p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <p className="text-sm font-semibold text-accent">
          {formatNumber(votingActivity.reduce((sum, point) => sum + point.votes, 0))} votes
        </p>
      </div>
      ...
      <div className="mt-4">
        <svg
          ...
          role="img"
          aria-label="Chart of votes recorded each day from Monday through Sunday"
        >
